...
export default function PackageSuppliers() {
  ...
  const [formData, setFormData] = useState<Omit<PackageSupplier, "id">>({
    name: "",
    email: "",
    phone: "",
    productDescription: "",
  ...
  useEffect(() => {
    const stored = localStorage.getItem("packageSuppliers");
    if (stored) {
      setSuppliers(JSON.parse(stored));
    }
  }, []);

  const saveSuppliers = (data: PackageSupplier[]) => {
    localStorage.setItem("packageSuppliers", JSON.stringify(data));
    setSuppliers(data);
  };
